...
The consent prompt is a native iOS/Android alert over the app, presented after preferences load and the app unlocks. It offers Privacy, No thanks and Allow analytics, with no preselected or emphasized consent option. Opening Privacy does not change consent; the alert returns after coming back from the browser and unlocking, if necessary. Navigation and the alert's own inactive/active transitions do not repeat the prompt. Copy is translated into all 31 supported languages. Consent is optional and can be changed under Settings → Privacy and data → Usage analytics. Refusal stops app events immediately, disables SDK collection and resets local Analytics data/instance ID. Delete-all first revokes Analytics. Previously uploaded events are not remotely deleted by a local reset.
