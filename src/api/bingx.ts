...
async function get<T>(
  ...
  return readJson<T>(url, { "X-BX-APIKEY": apiKey });
}
...
export async function fetchSpotWallet(
  ...
  { data: BingXSpotWallet; error?: never } | { data?: never; error: ApiError }
> {
  const ts = Date.now();
...
export async function fetchFuturesWallet(
  ...
  | { data: BingXFuturesWallet; error?: never }
...
export async function fetchSpotTrades(
  ...
  | { data: BingXSpotTradeResponse; error?: never }
  | { data?: never; error: ApiError }
> {
  const ts = Date.now();
...
export async function fetchFuturesPositions(
  ...
  | { data: BingXFuturesPositionResponse; error?: never }
  | { data?: never; error: ApiError }
> {
  const ts = Date.now();
  const query = `timestamp=${ts}&recvWindow=${RECV_WINDOW}`;
...
export async function fetchSpotTicker(
  ...
  | { data: BingXTickerResponse; error?: never }
  | { data?: never; error: ApiError }
> {
  const url = `${BASE}/openApi/spot/v1/ticker/price?symbol=${symbol}`;
  return readJson<BingXTickerResponse>(url);
}
