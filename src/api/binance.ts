...
export async function fetchWallets(
  ...
  | { data: BinanceWalletItem[]; error?: never }
...
export async function fetchSpotAccount(
  ...
  | { data: BinanceSpotAccount; error?: never }
  | { data?: never; error: ApiError }
> {
  const ts = Date.now();
...
export async function fetchSpotTrades(
  ...
  | { data: BinanceSpotTrade[]; error?: never }
  | { data?: never; error: ApiError }
> {
  const ts = Date.now();
...
export async function fetchFuturesPositions(
  ...
  | { data: BinanceFuturesPosition[]; error?: never }
  | { data?: never; error: ApiError }
> {
  const ts = Date.now();
  const query = `timestamp=${ts}&recvWindow=${RECV_WINDOW}`;
...
export async function fetchPriceTicker(
  ...
  | { data: BinancePriceTicker; error?: never }
  | { data?: never; error: ApiError }
> {
...
export function fetchApiRestrictions(keys: APIKeys) {
  ...
  return get<{
    ...
    `${BASE}/sapi/v1/account/apiRestrictions?${query}&signature=${sign(query, keys.secretKey)}`,
